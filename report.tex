...
\item A class depends on all classes defined in an accessed class. %TODO: If and acces can't be reloved?
...
If we look at figure \ref{fig:importRootPackage}, P.B.b could be accessed without first importing P since the model A, which P.B.b is used in, is in the same enclosing package as the model B.

But now we instead look at figure \ref{fig:newPackageWithoutImport}, where P not is imported and there is a new package also named P in the package P2. When the new package P.P2.P is added pi won't be found. When trying to lookup P.B.b the package P.P2.P will be found and it doesn't contain a class with the name B, so the lookup will fail.

If P had been imported the lookup of P.B.b had succeeded since an import starts at a root level package. This is the reason for the imports of the root package that are unnecessary at the moment but guarantees the right class or component always will be found. Even if someone in the future adds a new package with same name as an existing package.
...
\begin{figure}
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.5\textwidth]{EPS-graphs/HXL_one_file.eps}}
...
\begin{figure}
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.5\textwidth]{EPS-graphs/MSL_one_file.eps}}
...
\begin{figure}
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.5\textwidth]{EPS-graphs/MSL_history_plot.eps}}
