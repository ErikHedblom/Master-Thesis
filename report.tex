...
Modelica uses dot notation like many other languages to access something inside an object. Consider the code snippet in figure \ref{fig:modelicaExample}, Model \texttt{A} can be accessed though \texttt{P1.P2.A}. We will call this an access. Depending on from which scope \texttt{A} is being accessed the access could also be \texttt{P2.A} or just \texttt{A}. An access using dot notation is basically a list of accesses and we will call all the accesses before the last access, the path. For instance, \texttt{P1.P2} is the path of \texttt{P1.P2.A}. We will also denote an access, where the first part of it is referencing the top package in the library, as a qualified name.
...
In figure \ref{fig:parentGraph} we also expect rule 1 to create a dependency from \texttt{C} to \texttt{A} due to \texttt{k} being located in \texttt{A}. However, the access \texttt{k} cannot be resolved and we will miss the dependency to \texttt{A}. In this case we can see that rule 2 can compensate by adding an indirect dependency from \texttt{C} to \texttt{A}. 
If the constant k in package \texttt{A} is changed it will affect model \texttt{C}. Therefore we need a dependency from \texttt{C} to \texttt{A}. With rule number 1 we get dependencies from \texttt{C} to \texttt{B} and from \texttt{B} to \texttt{A}. So we have an indirect dependency from \texttt{C} to \texttt{A}.

Another way to think about rule 2 is to consider what happens if \texttt{A} or \texttt{B} is removed. The \texttt{C} can no longer be accessed since the path to \texttt{C} is broken. This will cause the compilation of C to fail.
...
\begin{figure}[!htbp]
    ...
    \subfloat{\raisebox{9.5 cm}{
        ...
        \end{minipage}}}
    \caption{Access with more access in its path, \texttt{A.f}}
    \label{fig:dotAccess}
\end{figure}

In figure \ref{fig:dotAccess}, we want dependencies from model \texttt{C} to all parts of the access to \texttt{f}. This means that in addition to a dependency to \texttt{f}, a dependency to \texttt{A} is also required. If package \texttt{A} is removed it will cause a compilation error when model \texttt{C} is compiled. This is caught by rule 3 and is why it is needed.
...
Rule 4 is needed to handle the case in figure \ref{fig:component}. Ideally the access \texttt{a.B.f} would create dependencies to \texttt{A}, \texttt{B} and \texttt{f}. Because \texttt{a} is a component, \texttt{P.A.B.f} cannot be resolved and now dependencies can be created. Instead rule 4 is used to create dependencies to everything defined in A when \texttt{a} is declared.
It is also possible that an access can not be fully resolved. This is the case in \ref{fig:brokenAccess}, where the access \texttt{P.a.f} cannot be resolved because the constant \texttt{a} is a component. In this case the only resolvable part is \texttt{P} and a  dependency is created to \texttt{P} by rule 3. \texttt{P} have dependencies to \texttt{A} and \texttt{f} through rule 4. Hence, it is not a problem that rule 4 cannot be applied to \texttt{P.a.f}.
