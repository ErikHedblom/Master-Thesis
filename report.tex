...
To make the test selection we have chosen to build a dependency graph for Modelica classes. We will produce a set of rules to be used when the dependency graph is built. The rules need to make sure that all dependencies between classes, that are need for a safe test selection, are included. We will probably not be able to reach full precision, it's likely that we need to include unnecessary dependencies to be keep the test selection safe.
