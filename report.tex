...
The next rule, rule 2, will create a dependency from a class to it's encapsulating class. An example can be found in figure \ref{fig:parentGraph}. In the example there is a constant k defined in package A. If another with the same name were to be added in in model B the reference to k in C will change to refer to the new constant. Therefore we need a dependency from C to B.

In figure \ref{fig:parentGraph} we also expect rule 1 to create a dependency from C to A due to k being located in A. However, the access k cannot be resolved and we will miss the dependency to A. In this case we can see that rule 2 can compensate by adding an indirect dependency from C to A. 
...
Another way to think about rule 2 is to consider what happens if A or B is removed. The C can no longer be accessed since the path to C is broken. This will cause the compilation of C to fail.
...
In figure \ref{fig:dotAccess}, we want dependencies from model C to all parts of the access to f. This means that in addition to a dependency to f a dependency to A is also required. If package A is removed it will cause a compilation error when model C is compiled. This is caught by rule 3 and is why it is needed.
...
But now we instead look at figure \ref{fig:newPackageWithoutImport}, where Modelica not is imported and there is a new package also named Modelica in the package Fluid. When the new package Modelica.Fluid.Modelica is added pi won't be found. When trying to lookup Modelica.Constants.pi the package Modelica.Fluid.Modelica will be found and it doesn't contain a class with the name Constants, so the lookup will fail.
